...
const CONFIG = {
  ...
  SPREADSHEET_ID: '15zfdrXZaR49HrVsHzpszLLbqFKCenvMn11IvMkYSMHI',
  SHEET_NAME: 'Teste Sistema Validado',
  PRICING_SHEET_NAME: 'Tabela de Preços Teste',
  ...
  CONTATOS: {
    ...
    HOTEL_EMAIL: 'emh@theempirehotels.com' // Email Empire Marques Hotel
  },
  ...
  NAMES: {
    HUB_OWNER: 'HUB Transfer Teste',
    ASSISTANT: 'Assistente Teste',
    HOTEL_MANAGER: 'Gerente Teste',
    HOTEL_NAME: 'Hotel Teste Validado',
    SISTEMA_NOME: 'Teste Sistema Validado e Hub Transfer'
  },
  ...
  SISTEMA: {
    VERSAO: '4.0-Teste-Sistema-Validado-Integrado',
    TIMEZONE: 'Europe/Lisbon',
    LOCALE: 'pt-PT',
    DATA_FORMATO: 'dd/mm/yyyy',
    HORA_FORMATO: 'HH:mm',
    ORGANIZAR_POR_MES: true,
    PREFIXO_MES: 'Teste_Transfers_',
    ANO_BASE: 2025,
...
const HEADERS = [
  ...
  'Referência'                   // U - Referência da reserva
];
...
const PRICING_HEADERS = [
  ...
  'Valor Hotel Teste Validado (€)', // K - Comissão hotel
...
const MESSAGES = {
  ...
  WHATSAPP_TEMPLATES: {
    NOVO_TRANSFER: {
      TITULO: '*🚐 NOVO TRANSFER TESTE-HUB*',
...
function validarDados(dados) {
  ...
  const outrosCampos = ['numeroVoo', 'origem', 'destino', 'pagoParaQuem', 'observacoes', 'tourSelecionado', 'referencia'];
...
function calcularPrecoInteligente(origem, destino, pessoas, bagagens, tipoServico = 'Transfer') {
 ...
 try {
   ...
   const locaisConhecidos = {
     ...
     'teste': { tipo: 'hotel', distanciaBase: 0 },
  ...
  try {
    ...
    switch (action) {
      ...
      default:
        ...
        return createJsonResponse({
          status: 'success',
          message: 'Sistema Empire Teste Hotel-HUB Transfer funcionando!',
          versao: CONFIG.SISTEMA.VERSAO,
          endpoints: {
...
function processarNovoTransfer(dadosRecebidos) {
  ...
  try {
    ...
    const dadosTransfer = [
      ...
      validarEFormatarHora(dados.horaPickup), // K - Hora Pick-up
      ...
      sanitizarTexto(dados.referencia || '') // U - Referência
    ];
...
function getAllData() {
  ...
  try {
    ...
    const totalColumns = Math.max(HEADERS.length, 21); // Garantir que leia até coluna U
    const range = sheet.getRange(2, 1, lastRow - 1, totalColumns);
...
function adicionarColunaReferencia() {
  console.log('Adicionando coluna Referência...');
  
  try {
    const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);
    
    // 1. Atualizar aba principal
    const abaPrincipal = ss.getSheetByName(CONFIG.SHEET_NAME);
    if (abaPrincipal) {
      // Verificar se já tem a coluna U
      const headers = abaPrincipal.getRange(1, 1, 1, abaPrincipal.getLastColumn()).getValues()[0];
      
      if (!headers.includes('Referência')) {
        // Adicionar header na coluna U
        abaPrincipal.getRange(1, 21).setValue('Referência');
        
        // Formatar header
        const headerCell = abaPrincipal.getRange(1, 21);
        headerCell
        ...
        abaPrincipal.setColumnWidth(21, 150);
        
        // Formatar coluna como texto
        abaPrincipal.getRange('U:U').setNumberFormat('@');
        
        console.log('✅ Coluna Referência adicionada na aba principal');
      }
    }
    
    // 2. Atualizar todas as abas mensais
    const sheets = ss.getSheets();
    let abasMensaisAtualizadas = 0;
    
    sheets.forEach(sheet => {
      const nome = sheet.getName();
      if (nome.startsWith('Transfers_') || nome.includes('2025')) {
        const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
        
        if (!headers.includes('Referência')) {
          sheet.getRange(1, 21).setValue('Referência');
          
          const headerCell = sheet.getRange(1, 21);
          headerCell
            .setBackground('#2c3e50')
            .setFontColor('#ffffff')
            .setFontWeight('bold')
          ...
          abasMensaisAtualizadas++;
        }
      }
    });
    
    console.log(`✅ ${abasMensaisAtualizadas} abas mensais atualizadas`);
    console.log('🎉 Coluna Referência criada com sucesso!');
    
    return {
      sucesso: true,
      abasAtualizadas: abasMensaisAtualizadas + 1
    };
