// ===================================================
// SISTEMA DE TRANSFERS GOTA D'AGUA & HUB TRANSFER v4.0
...
const CONFIG = {
  ...
  CONTATOS: {
    ...
    HOTEL_PHONE: '+351210548700',      // Telefone Gota d'agua
    HOTEL_EMAIL: 'robertavgutierez@gmail.com' // Email Gota d'agua
  },
  ...
  NAMES: {
    ...
    HOTEL_MANAGER: 'Luiz Felipe',
    HOTEL_NAME: 'Gota d agua',
    SISTEMA_NOME: 'Sistema Gota d agua e Hub Transfer'
  },
  
// 💰 Valores Padrão e Configurações Financeiras (SISTEMA DE VALORES FIXOS)
  VALORES: {
    VALOR_BASE_AEROPORTO: 35.00,      // Valor base Gota d'água ↔ Aeroporto
    COMISSAO_GOTA_DAGUA: 6.50,        // Comissão fixa Gota d'água
    COMISSAO_HUB_TRANSFER: 28.50,     // Comissão fixa HUB Transfer
    MOEDA: '€',
    FORMATO_MOEDA: '€#,##0.00'
    // REMOVIDOS: HOTEL_PADRAO, HUB_PADRAO, PERCENTUAL_HOTEL, PERCENTUAL_HUB, COMISSAO_RECEPCAO_TOUR, COMISSAO_RECEPCAO_TRANSFER
  },
  
  // 🌐 Configurações do Sistema (FUSÃO)
  SISTEMA: {
    VERSAO: '4.0-Gota-d-agua-Hub-Transfer-Integrado',
...
const HEADERS = [
  ...
  'Valor Gota d agua (€)', // M - Comissão do hotel
...
const PRICING_HEADERS = [
  ...
  'Valor Gota d agua (€)', // K - Comissão hotel
...
const MESSAGES = {
  ...
  WHATSAPP_TEMPLATES: {
    NOVO_TRANSFER: {
      TITULO: '*🚐 NOVO TRANSFER Gotadagua-HUB*',
...
/**
 * Calcula valores do transfer baseado no sistema de valores fixos da Gota d'água
 * NOVA LÓGICA: Sistema de valores fixos por rota específica
...
function calcularValores(origem, destino, pessoas, bagagens, precoManual = null, tipoServico = 'Transfer') {
  ...
  try {
    ...
    // Estratégia 3: Verificar se é rota Aeroporto ↔ Gota d'água (VALORES FIXOS PADRÃO)
    const rotaAeroporto = verificarRotaAeroporto(origem, destino);
    if (rotaAeroporto) {
      return calcularValoresRotaAeroporto();
    }
    
    // Estratégia 4: Calcular com base em distância/complexidade
    const precoCalculado = calcularPrecoInteligente(origem, destino, pessoas, bagagens, tipoServico);
    if (precoCalculado) {
      return precoCalculado;
    }
    
    // Estratégia 5: Valores padrão baseados no tipo de serviço
...
/**
 * Verifica se é uma rota Aeroporto ↔ Gota d'água
 * @private
 */
function verificarRotaAeroporto(origem, destino) {
  const origemLower = origem.toLowerCase();
  const destinoLower = destino.toLowerCase();
  const hotelLower = CONFIG.NAMES.HOTEL_NAME.toLowerCase();
  
  const isAeroportoOrigem = origemLower.includes('aeroporto') || origemLower.includes('airport');
  const isAeroportoDestino = destinoLower.includes('aeroporto') || destinoLower.includes('airport');
  const isHotelOrigem = origemLower.includes('gota') || origemLower.includes(hotelLower);
  const isHotelDestino = destinoLower.includes('gota') || destinoLower.includes(hotelLower);
  
  return (isAeroportoOrigem && isHotelDestino) || (isHotelOrigem && isAeroportoDestino);
}

/**
 * Calcula valores fixos para rota Aeroporto ↔ Gota d'água
 * @private
 */
function calcularValoresRotaAeroporto() {
  const precoCliente = CONFIG.VALORES.VALOR_BASE_AEROPORTO;
  const valorHotel = CONFIG.VALORES.COMISSAO_GOTA_DAGUA;
  const valorHUB = CONFIG.VALORES.COMISSAO_HUB_TRANSFER;
  
  return {
    precoCliente: precoCliente,
    valorHotel: valorHotel,
    valorHUB: valorHUB,
    comissaoRecepcao: 0, // Sem comissão separada no novo sistema
    fonte: 'valores-fixos-aeroporto',
    tipoServico: 'Transfer',
    observacoes: `Valores fixos - Aeroporto ↔ Gota d'água - €${valorHotel} Gota d'água + €${valorHUB} HUB`
  };
}

/**
 * Calcula valores quando preço manual é informado (SISTEMA DE VALORES FIXOS)
 * @private
 */
function calcularComPrecoManual(origem, destino, pessoas, bagagens, precoManual, tipoServico) {
  ...
  // Tentar encontrar valores fixos na tabela
  const precoTabela = consultarPrecoTabela(origem, destino, pessoas, bagagens, tipoServico);
  
  if (precoTabela && precoTabela.precoCliente > 0) {
    // Usar valores fixos da tabela (não proporcionais)
    return {
      precoCliente: precoCliente,
      valorHotel: precoTabela.valorHotel, // Valor fixo da tabela
      valorHUB: precoTabela.valorHUB,     // Valor fixo da tabela
      comissaoRecepcao: precoTabela.comissaoRecepcao || 0,
      fonte: 'manual-com-tabela-fixa',
      tipoServico: tipoServico,
      observacoes: `Preço manual com valores fixos da tabela`
    };
  }
  
// SUBSTITUIR POR (código novo):
return calcularPorTipoServicoFixo(precoCliente, tipoServico);
}

/**
 * Calcula valores baseado no tipo de serviço (SISTEMA DE VALORES FIXOS)
 * FUNÇÃO PRINCIPAL DO NOVO SISTEMA - ADICIONAR OBRIGATORIAMENTE
 * @private
 */
function calcularPorTipoServicoFixo(precoCliente, tipoServico) {
  let valorHotel, valorHUB, comissaoRecepcao = 0;
  
  // Sistema de valores fixos baseado no tipo de serviço
  if (tipoServico === 'Tour Regular' || tipoServico === 'tour') {
    // Tours regulares: usar valores fixos ou proporção específica
    valorHotel = CONFIG.VALORES.COMISSAO_GOTA_DAGUA || (precoCliente * 0.20);
    valorHUB = precoCliente - valorHotel;
    
  } else if (tipoServico === 'Private Tour' || tipoServico === 'private') {
    // Private tours: usar valores fixos ou proporção específica
    valorHotel = CONFIG.VALORES.COMISSAO_GOTA_DAGUA || (precoCliente * 0.20);
    valorHUB = precoCliente - valorHUB;
    
  } else {
    // Transfers: usar valores fixos definidos
    if (precoCliente === CONFIG.VALORES.VALOR_BASE_AEROPORTO) {
      // Rota padrão aeroporto
      valorHotel = CONFIG.VALORES.COMISSAO_GOTA_DAGUA;
      valorHUB = CONFIG.VALORES.COMISSAO_HUB_TRANSFER;
    } else {
      // Outras rotas: distribuir proporcionalmente mantendo a lógica fixa
      const proporcaoHotel = CONFIG.VALORES.COMISSAO_GOTA_DAGUA / CONFIG.VALORES.VALOR_BASE_AEROPORTO;
      valorHotel = Math.round(precoCliente * proporcaoHotel * 100) / 100;
      valorHUB = precoCliente - valorHotel;
    }
  }
  ...
  return {
    ...
    fonte: 'valores-fixos-tipo-servico',
    tipoServico: tipoServico,
    observacoes: `Valores fixos - ${tipoServico} - €${valorHotel} Gota d'água + €${valorHUB} HUB`
  };
}
...
/**
 * FUNÇÃO TEMPORÁRIA DE CORREÇÃO
 * Resolve o erro "calcularPorTipoServico is not defined"
 * Adicionar no final do código até encontrarmos todas as ocorrências
 */
function calcularPorTipoServico(precoCliente, tipoServico) {
  logger.debug('Redirecionando calcularPorTipoServico para calcularPorTipoServicoFixo');
...
/**
 * Calcula valores padrão baseados no tipo de serviço (SISTEMA DE VALORES FIXOS)
 * @private
 */
function calcularValoresPadrao(tipoServico, pessoas) {
  ...
  switch (tipoServico) {
    ...
    case 'private':
      ...
      break;
    default:
      precoCliente = CONFIG.VALORES.VALOR_BASE_AEROPORTO; // Valor base padrão
  }
...
/**
 * Consulta preço na tabela com algoritmo de matching inteligente (PRESERVADO)
 * @private
 */
...
/**
 * Calcula score de matching entre busca e registro da tabela (PRESERVADO)
 * @private
 */
...
/**
 * Calcula preço de forma inteligente baseado em regras de negócio (ADAPTADO PARA GOTA D'ÁGUA)
 * @private
 */
function calcularPrecoInteligente(origem, destino, pessoas, bagagens, tipoServico = 'Transfer') {
  ...
  try {
    // Base de conhecimento de locais e distâncias aproximadas (ADAPTADO PARA COSTA DA CAPARICA)
    const locaisConhecidos = {
      'aeroporto': { tipo: 'aeroporto', distanciaBase: 25 },
      'lisboa': { tipo: 'cidade', distanciaBase: 25 },
      'cascais': { tipo: 'cidade', distanciaBase: 40 },
      'sintra': { tipo: 'cidade', distanciaBase: 50 },
      'caparica': { tipo: 'hotel', distanciaBase: 0 },
      'costa da caparica': { tipo: 'hotel', distanciaBase: 0 },
      'gota': { tipo: 'hotel', distanciaBase: 0 },
    ...
    let distanciaEstimada = 30; // km padrão
    ...
    if (tipoServico === 'Tour Regular' || tipoServico === 'tour') {
      ...
      // Transfers: usar valor base da Gota d'água como referência
      precoBase = CONFIG.VALORES.VALOR_BASE_AEROPORTO;
      
      // Ajustar por distância (€1.00 por km adicional)
      if (distanciaEstimada > 25) {
        precoBase += (distanciaEstimada - 25) * 1.00;
      }
      
      // Adicionar por pessoa extra (€5 por pessoa além da primeira)
      const pessoasInt = parseInt(pessoas) || 1;
      if (pessoasInt > 1) {
        precoBase += (pessoasInt - 1) * 5;
      }
      
      // Adicionar por bagagem extra (€3 por bagagem além de 1 por pessoa)
      const bagagensInt = parseInt(bagagens) || 0;
      const bagagensExtras = Math.max(0, bagagensInt - pessoasInt);
      if (bagagensExtras > 0) {
        precoBase += bagagensExtras * 3;
      }
    }
    ...
    // 🔧 CORREÇÃO: Usar função com nome correto
    const valores = calcularPorTipoServicoFixo(precoCliente, tipoServico);
...
/**
 * Cria HTML do e-mail interativo - VERSÃO LIMPA SEM COMISSÃO RECEPÇÃO
 * CORREÇÕES: Sem ícones problemáticos, apenas títulos + data correta + SEM comissão recepção
 * @private
 */
...
/**
 * Cria HTML do e-mail de relatório diário (VERSÃO PROFISSIONAL - SEM RECEPÇÃO)
 */
          ...
          <div class="financial-summary">
            ...
            <div class="financial-grid">
              <div class="financial-item">
                ...
                <div class="financial-label">${CONFIG.NAMES.HOTEL_NAME}</div>
              </div>
              <div class="financial-item">
...
// ===================================================
// GESTÃO DA TABELA DE PREÇOS - SISTEMA HÍBRIDO
// PREÇO FIXO: €35 (€6.50 Gota d'água + €28.50 HUB) COMO PADRÃO
// PERSONALIZADO: Permite valores customizados quando necessário
// ===================================================

/**
 * Adiciona novo preço à tabela (SISTEMA HÍBRIDO - FIXO + PERSONALIZADO)
...
function adicionarPrecoTabela(dadosPreco) {
  ...
  try {
    ...
    const camposObrigatorios = ['origem', 'destino'];
    ...
    // ========================================
    // SISTEMA HÍBRIDO: PREÇO FIXO OU PERSONALIZADO
    // ========================================
    ...
    const VALORES_PADRAO = {
      ...
      valorGotaDagua: 6.50,
      valorHUB: 28.50,
      comissaoRecepcao: 0.00
    };
    
    // Determinar se usar valores fixos ou personalizados
    let valorHotel, valorHUB, precoCliente, comissaoRecepcao;
    let observacaoSistema = '';
    
    if (dadosPreco.precoCliente && 
        (dadosPreco.valorHotel || dadosPreco.valorHUB)) {
      // MODO PERSONALIZADO - usar valores fornecidos
      precoCliente = parseFloat(dadosPreco.precoCliente);
      valorHotel = parseFloat(dadosPreco.valorHotel) || 0;
      valorHUB = parseFloat(dadosPreco.valorHUB) || 0;
      comissaoRecepcao = parseFloat(dadosPreco.comissaoRecepcao) || 0;
      observacaoSistema = `Preço personalizado: €${valorHotel} Gota d'água + €${valorHUB} HUB = €${precoCliente}`;
      
      logger.info('Usando valores personalizados', { precoCliente, valorHotel, valorHUB });
      
    } else if (dadosPreco.tipoServico && dadosPreco.precoCliente) {
      // MODO CÁLCULO AUTOMÁTICO - usar função existente
      const valores = calcularPorTipoServico(
        dadosPreco.precoCliente, 
        dadosPreco.tipoServico
      );
      precoCliente = parseFloat(dadosPreco.precoCliente);
      valorHotel = valores.valorHotel;
      valorHUB = valores.valorHUB;
      comissaoRecepcao = valores.comissaoRecepcao;
      observacaoSistema = `Calculado automaticamente por tipo de serviço: ${dadosPreco.tipoServico}`;
      
      logger.info('Usando cálculo automático', { tipoServico: dadosPreco.tipoServico, valores });
      
    } else {
      // MODO PADRÃO - usar valores fixos
      precoCliente = VALORES_PADRAO.precoCliente;
      valorHotel = VALORES_PADRAO.valorGotaDagua;
      valorHUB = VALORES_PADRAO.valorHUB;
      comissaoRecepcao = VALORES_PADRAO.comissaoRecepcao;
      observacaoSistema = `Preço fixo padrão: €${valorHotel} Gota d'água + €${valorHUB} HUB = €${precoCliente}`;
      
      logger.info('Usando valores fixos padrão', VALORES_PADRAO);
    }
    
    // Montar linha para inserção
    const novaLinha = [
      novoId,                                      // A - ID
      ...
      precoCliente,                                // J - Preço Cliente
      valorHotel,                                  // K - Valor Hotel (Gota d'água)
      valorHUB,                                    // L - Valor HUB
      comissaoRecepcao,                            // M - Comissão Recepção
      dadosPreco.ativo !== false ? 'Sim' : 'Não', // N - Ativo
      new Date(),                                  // O - Data Criação
      dadosPreco.observacoes || observacaoSistema  // P - Observações
    ];
    
    // Inserir na planilha
    sheet.appendRow(novaLinha);
    
    logger.success('Preço adicionado com sucesso', { 
      id: novoId,
      rota: `${dadosPreco.origem} → ${dadosPreco.destino}`,
      precoCliente,
      valorHotel,
      valorHUB
    });
    ...
    return {
      ...
      valores: {
        ...
        gotaDagua: valorHotel,
        hubTransfer: valorHUB,
        comissaoRecepcao: comissaoRecepcao,
        tipo: dadosPreco.precoCliente ? 'personalizado' : 'padrao'
      }
    };
...
/**
 * Atualiza preço existente na tabela (PRESERVANDO FUNCIONALIDADE ORIGINAL + MELHORIAS)
...
function atualizarPrecoTabela(id, dadosAtualizacao) {
  ...
  try {
    ...
    // Atualizar campos específicos (ESTRUTURA DO CÓDIGO ORIGINAL)
...
/**
 * Adiciona preço fixo personalizado ao sistema (FUNCIONALIDADE ORIGINAL PRESERVADA)
 * @param {string} origem - Local de origem
 * @param {string} destino - Local de destino
 * @param {number} valorGotaDagua - Valor fixo para Gota d'água Surf Camps
 * @param {number} valorHUB - Valor fixo para HUB Transfer
 * @param {Object} opcoes - Opções adicionais (opcional)
 * @returns {Object} - Resultado da operação
 */
function adicionarPrecoFixoPersonalizado(origem, destino, valorGotaDagua, valorHUB, opcoes = {}) {
  logger.info('Adicionando preço fixo personalizado', {
    origem, destino, valorGotaDagua, valorHUB, opcoes
  });
  
  try {
    // Validar entradas
    const validacao = validarEntradasPrecoFixo(origem, destino, valorGotaDagua, valorHUB, opcoes);
    if (!validacao.valido) {
      throw new Error(`Dados inválidos: ${validacao.erros.join(', ')}`);
    }
    
    // Calcular valor total
    const valorTotal = parseFloat(valorGotaDagua) + parseFloat(valorHUB);
    
    // Preparar dados para tabela
    const dadosPreco = {
      tipoServico: opcoes.tipoServico || 'Transfer',
      origem: sanitizarTexto(origem),
      destino: sanitizarTexto(destino),
      pessoas: parseInt(opcoes.pessoas) || 1,
      bagagens: parseInt(opcoes.bagagens) || 0,
      precoPorPessoa: opcoes.tipoServico === 'Tour Regular' ? valorTotal : 0,
      precoPorGrupo: opcoes.tipoServico === 'Private Tour' ? valorTotal : 0,
      precoCliente: valorTotal,
      valorHotel: parseFloat(valorGotaDagua),
      valorHUB: parseFloat(valorHUB),
      comissaoRecepcao: 0,
      observacoes: opcoes.observacoes || `Preço fixo personalizado - €${valorGotaDagua} Gota d'água + €${valorHUB} HUB`,
      ativo: opcoes.ativo !== false
    };
    
    // Adicionar à tabela
    const resultadoTabela = adicionarPrecoTabela(dadosPreco);
    
    if (!resultadoTabela.sucesso) {
      throw new Error(`Erro ao adicionar na tabela: ${resultadoTabela.erro}`);
    }
    
    // Aplicar ao sistema
    const aplicacaoSistema = aplicarPrecoAoSistema(dadosPreco, resultadoTabela.id);
    
    // Log
    const rotaDescricao = `${origem} → ${destino}`;
    
    logger.success('Preço fixo personalizado adicionado com sucesso', {
      id: resultadoTabela.id,
      rota: rotaDescricao,
      valorTotal: valorTotal,
      distribuicao: { gotaDagua: valorGotaDagua, hub: valorHUB }
    });
    
    // Notificação por e-mail (opcional)
    if (opcoes.notificarPorEmail !== false) {
      try {
        enviarNotificacaoNovoPreco(dadosPreco, resultadoTabela.id);
      } catch (emailError) {
        logger.warn('Erro ao enviar notificação por e-mail', emailError);
      }
    }
    ...
    return {
      ...
      valores: {
        total: valorTotal,
        gotaDagua: valorGotaDagua,
        hub: valorHUB,
        moeda: CONFIG.VALORES.MOEDA
      },
      configuracao: dadosPreco,
      aplicadoAoSistema: aplicacaoSistema.sucesso,
      mensagem: `Preço fixo adicionado: ${rotaDescricao} - Total: €${valorTotal} (€${valorGotaDagua} + €${valorHUB})`,
      timestamp: new Date()
    };
    
  } catch (error) {
    logger.error('Erro ao adicionar preço fixo personalizado', error);
    ...
    return {
      ...
      origem: origem,
      destino: destino,
      valores: { gotaDagua: valorGotaDagua, hub: valorHUB },
      timestamp: new Date()
    };
  }
}
...
/**
 * NOVA FUNÇÃO: Adiciona rota com preço fixo padrão (€35)
 * @param {string} origem - Local de origem
 * @param {string} destino - Local de destino
 * @param {Object} opcoes - Opções adicionais (opcional)
 * @returns {Object} - Resultado da operação
 */
function adicionarRotaPrecoFixo(origem, destino, opcoes = {}) {
  logger.info('Adicionando rota com preço fixo padrão', { origem, destino });
  
  const dadosPreco = {
    tipoServico: opcoes.tipoServico || 'Transfer',
    origem: origem,
    destino: destino,
    pessoas: opcoes.pessoas || 1,
    bagagens: opcoes.bagagens || 0,
    observacoes: opcoes.observacoes || 'Preço fixo padrão para todas as rotas'
    // Não definir precoCliente para usar valores padrão
  };
  
  return adicionarPrecoTabela(dadosPreco);
}

/**
 * NOVA FUNÇÃO: Menu do Google Sheets para adicionar rota com preço fixo
 */
function adicionarRotaViaMenu() {
  const ui = SpreadsheetApp.getUi();
  
  try {
    // Mostrar opções de preço
    const opcaoPreco = ui.alert(
      '💰 Tipo de Preço',
      'Escolha o tipo de preço:\n\n' +
      '• FIXO: €35 (€6,50 Gota d\'água + €28,50 HUB)\n' +
      '• PERSONALIZADO: Valores customizados\n\n' +
      'Usar preço FIXO padrão?',
      ui.ButtonSet.YES_NO_CANCEL
    );
    
    if (opcaoPreco === ui.Button.CANCEL) return;
    ...
    if (opcaoPreco === ui.Button.YES) {
      // PREÇO FIXO
      resultado = adicionarRotaPrecoFixo(
      ...
      if (resultado.sucesso) {
        ui.alert(
          '✅ Rota Adicionada com Preço Fixo!',
          `Rota: ${origem.getResponseText()} → ${destino.getResponseText()}\n\n` +
          `💰 Valores FIXOS:\n` +
          `• Cliente paga: €${resultado.valores.cliente}\n` +
          `• Gota d'água: €${resultado.valores.gotaDagua}\n` +
          `• HUB Transfer: €${resultado.valores.hubTransfer}\n\n` +
          `ID: #${resultado.id}`,
          ui.ButtonSet.OK
        );
      }
      ...
      // PREÇO PERSONALIZADO
      const valorGota = ui.prompt('💰 Valor Gota d\'água', 'Digite o valor para Gota d\'água Surf Camps (€):', ui.ButtonSet.OK_CANCEL);
      if (valorGota.getSelectedButton() !== ui.Button.OK) return;
      
      const valorHUB = ui.prompt('🚐 Valor HUB', 'Digite o valor para HUB Transfer (€):', ui.ButtonSet.OK_CANCEL);
      if (valorHUB.getSelectedButton() !== ui.Button.OK) return;
      
      resultado = adicionarPrecoFixoPersonalizado(
        origem.getResponseText(),
        destino.getResponseText(),
        parseFloat(valorGota.getResponseText()),
        parseFloat(valorHUB.getResponseText())
      );
      
      if (resultado.sucesso) {
        ui.alert(
          '✅ Preço Personalizado Adicionado!',
          resultado.mensagem + `\n\nID: #${resultado.id}`,
          ui.ButtonSet.OK
        );
      }
...
/**
 * NOVA FUNÇÃO: Adiciona as rotas principais com preço fixo
 */
function adicionarRotasPrincipais() {
  logger.info('Adicionando rotas principais com preço fixo');
  
  const rotasPrincipais = [
    { origem: 'Aeroporto de Lisboa', destino: 'Gota d\'água Surf Camps' },
    { origem: 'Gota d\'água Surf Camps', destino: 'Aeroporto de Lisboa' },
    { origem: 'Lisboa Centro', destino: 'Costa da Caparica' },
    { origem: 'Costa da Caparica', destino: 'Lisboa Centro' },
    { origem: 'Aeroporto de Lisboa', destino: 'Costa da Caparica' },
    { origem: 'Costa da Caparica', destino: 'Aeroporto de Lisboa' }
  ];
  ...
  rotasPrincipais.forEach(rota => {
    try {
      const resultado = adicionarRotaPrecoFixo(rota.origem, rota.destino, {
        observacoes: 'Rota principal - preço fixo padrão'
      });
  ...
  logger.success('Rotas principais adicionadas', { sucessos, erros });
  ...
  return {
    ...
    erros: erros,
    resultados: resultados,
    mensagem: `${sucessos} rotas adicionadas com sucesso, ${erros} erros`
  };
}

/**
 * Remove preço da tabela (soft delete - marca como inativo) - FUNCIONALIDADE PRESERVADA
 * @param {number} id - ID do preço
 * @returns {Object} - Resultado da operação
 */
...
/**
 * Lista todos os preços da tabela (FUNCIONALIDADE ORIGINAL PRESERVADA + MELHORIAS)
...
/**
 * Cria a tabela de preços se não existir (FUNCIONALIDADE ORIGINAL PRESERVADA + ADAPTADA)
 * @private
 */
...
// ===================================================
// FUNÇÕES AUXILIARES (FUNCIONALIDADES ORIGINAIS PRESERVADAS)
// ===================================================

/**
 * Valida entradas para preço fixo personalizado (FUNCIONALIDADE ORIGINAL)
 * @private
 */
function validarEntradasPrecoFixo(origem, destino, valorGotaDagua, valorHUB, opcoes) {
  const erros = [];
  ...
  const valorGotaNum = parseFloat(valorGotaDagua);
  const valorHUBNum = parseFloat(valorHUB);
  
  if (isNaN(valorGotaNum) || valorGotaNum <= 0) {
    erros.push('Valor Gota d\'água deve ser um número positivo');
  }
  
  if (isNaN(valorHUBNum) || valorHUBNum <= 0) {
    erros.push('Valor HUB deve ser um número positivo');
  }
  
  if (valorGotaNum > CONFIG.LIMITES.MAX_VALOR || valorHUBNum > CONFIG.LIMITES.MAX_VALOR) {
    erros.push(`Valores não podem exceder €${CONFIG.LIMITES.MAX_VALOR}`);
  }
  
  // Validar total
  const total = valorGotaNum + valorHUBNum;
  if (total > CONFIG.LIMITES.MAX_VALOR) {
    erros.push(`Valor total (€${total}) excede o limite máximo (€${CONFIG.LIMITES.MAX_VALOR})`);
  }
  
  // Validar opções se fornecidas
  if (opcoes.pessoas && (isNaN(opcoes.pessoas) || opcoes.pessoas < 1 || opcoes.pessoas > CONFIG.LIMITES.MAX_PESSOAS)) {
    erros.push(`Número de pessoas deve estar entre 1 e ${CONFIG.LIMITES.MAX_PESSOAS}`);
  }
  
  if (opcoes.bagagens && (isNaN(opcoes.bagagens) || opcoes.bagagens < 0 || opcoes.bagagens > CONFIG.LIMITES.MAX_BAGAGENS)) {
    erros.push(`Número de bagagens deve estar entre 0 e ${CONFIG.LIMITES.MAX_BAGAGENS}`);
  }
  
  if (opcoes.tipoServico && !VALIDACOES.VALORES_PERMITIDOS.TIPO_SERVICO.includes(opcoes.tipoServico)) {
    erros.push('Tipo de serviço inválido');
  }
...
/**
 * Aplica o novo preço imediatamente ao sistema (FUNCIONALIDADE ORIGINAL)
 * @private
 */
function aplicarPrecoAoSistema(dadosPreco, precoId) {
  try {
    // Invalidar cache de preços se existir
    if (typeof invalidarCachePrecos === 'function') {
      invalidarCachePrecos();
    }
    
    // Registrar no log do sistema
    logger.info('Novo preço aplicado ao sistema', {
      id: precoId,
      rota: `${dadosPreco.origem} → ${dadosPreco.destino}`,
      ativo: true
    });
    
    // Testar se o preço pode ser encontrado imediatamente
    const teste = consultarPrecoTabela(
      dadosPreco.origem, 
      dadosPreco.destino, 
      dadosPreco.pessoas, 
      dadosPreco.bagagens, 
    ...
    const encontrado = teste && teste.id === precoId;
    
    return {
      sucesso: true,
      encontradoEmTeste: encontrado,
      cacheInvalidado: true
    };
    
  } catch (error) {
    logger.error('Erro ao aplicar preço ao sistema', error);
    return {
      sucesso: false,
      erro: error.message
    };
...
/**
 * Envia notificação por e-mail sobre novo preço (FUNCIONALIDADE ORIGINAL)
 * @private
 */
function enviarNotificacaoNovoPreco(dadosPreco, precoId) {
  try {
    const destinatarios = CONFIG.EMAIL_CONFIG.DESTINATARIOS.join(',');
    const assunto = `[${CONFIG.NAMES.SISTEMA_NOME}] Novo Preço Fixo Adicionado`;
    const rota = `${dadosPreco.origem} → ${dadosPreco.destino}`;
    
    const corpo = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2c3e50;">💰 Novo Preço Fixo Personalizado</h2>
        
        <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <h3>📍 Rota: ${rota}</h3>
          <p><strong>ID do Preço:</strong> #${precoId}</p>
          <p><strong>Tipo de Serviço:</strong> ${dadosPreco.tipoServico}</p>
          <p><strong>Pessoas:</strong> ${dadosPreco.pessoas}</p>
          <p><strong>Bagagens:</strong> ${dadosPreco.bagagens}</p>
        </div>
        
        <div style="background: #e8f5e8; padding: 20px; border-radius: 8px; margin: 20px 0;">
          <h3>💶 Distribuição de Valores:</h3>
          <p><strong>Valor Total:</strong> €${dadosPreco.precoCliente.toFixed(2)}</p>
          <p><strong>Gota d'água Surf Camps:</strong> €${dadosPreco.valorHotel.toFixed(2)}</p>
          <p><strong>HUB Transfer:</strong> €${dadosPreco.valorHUB.toFixed(2)}</p>
        </div>
        
        ${dadosPreco.observacoes ? `
        <div style="background: #fff3cd; padding: 15px; border-radius: 6px; margin: 20px 0;">
          <p><strong>Observações:</strong> ${dadosPreco.observacoes}</p>
        </div>
        ` : ''}
        
        <p style="color: #666; font-size: 14px;">
          Este preço foi adicionado automaticamente ao sistema e já está disponível para uso.
        </p>
        
        <p><strong>Data de Criação:</strong> ${formatarDataHora(new Date())}</p>
      </div>
    `;
    ...
    logger.success('Notificação de novo preço enviada por e-mail');
    
  } catch (error) {
    logger.error('Erro ao enviar notificação de novo preço', error);
    throw error;
  }
}

/**
 * Importa preços de uma fonte externa (FUNCIONALIDADE ORIGINAL PRESERVADA)
...
/**
 * Exporta tabela de preços (FUNCIONALIDADE ORIGINAL PRESERVADA)
...
// ===================================================
// FUNÇÕES DE MANUTENÇÃO E UTILITÁRIOS (FUNCIONALIDADES ORIGINAIS PRESERVADAS)
// ===================================================

/**
 * Limpa todos os dados do sistema (FUNCIONALIDADE ORIGINAL PRESERVADA)
...
/**
 * Limpa apenas dados de teste (FUNCIONALIDADE ORIGINAL PRESERVADA)
 * @returns {Object} - Resultado da limpeza
 */
...
// ===================================================
// NOVAS FUNÇÕES DE TESTE E CONFIGURAÇÃO
// ===================================================

/**
 * NOVA FUNÇÃO: Função de teste para validar o sistema híbrido
 */
function testarSistemaPrecoHibrido() {
  console.log('🧪 TESTANDO SISTEMA HÍBRIDO DE PREÇOS...');
  
  try {
    // Teste 1: Preço fixo padrão (€35)
    const teste1 = adicionarRotaPrecoFixo('Aeroporto de Lisboa', 'Gota d\'água Surf Camps');
    console.log('✅ Teste 1 - Preço Fixo Padrão:', teste1.sucesso ? 'SUCESSO' : 'ERRO');
    if (teste1.sucesso) {
      console.log(`   Valores: Cliente €${teste1.valores?.cliente} | Gota d'água €${teste1.valores?.gotaDagua} | HUB €${teste1.valores?.hubTransfer}`);
    }
    
    // Teste 2: Preço personalizado
    const teste2 = adicionarPrecoFixoPersonalizado('Lisboa Centro', 'Sintra', 10.00, 25.00);
    console.log('✅ Teste 2 - Preço Personalizado:', teste2.sucesso ? 'SUCESSO' : 'ERRO');
    if (teste2.sucesso) {
      console.log(`   Valores: Total €${teste2.valores?.total} | Gota d'água €${teste2.valores?.gotaDagua} | HUB €${teste2.valores?.hub}`);
    }
    
    // Teste 3: Preço calculado automaticamente
    const teste3 = adicionarPrecoTabela({
      origem: 'Porto',
      destino: 'Lisboa',
      tipoServico: 'Transfer',
      precoCliente: 50.00
    });
    console.log('✅ Teste 3 - Cálculo Automático:', teste3.sucesso ? 'SUCESSO' : 'ERRO');
    if (teste3.sucesso) {
      console.log(`   Tipo: ${teste3.valores?.tipo}`);
    }
    
    // Teste 4: Listar todos os preços
    const precos = listarPrecosTabela({ apenasAtivos: true });
    console.log('✅ Teste 4 - Listagem:', precos.length > 0 ? 'SUCESSO' : 'ERRO');
    console.log(`   Total de preços: ${precos.length}`);
    
    // Verificar diversidade de preços
    const precosUnicos = [...new Set(precos.map(p => p.precoCliente))];
    console.log(`   Preços únicos: ${precosUnicos.length}`);
    
    console.log('');
    console.log('🎯 RESUMO DOS TESTES:');
    console.log(`• Preço fixo padrão: ${teste1.sucesso ? '✅' : '❌'}`);
    console.log(`• Preço personalizado: ${teste2.sucesso ? '✅' : '❌'}`);
    console.log(`• Cálculo automático: ${teste3.sucesso ? '✅' : '❌'}`);
    console.log(`• Listagem: ${precos.length > 0 ? '✅' : '❌'}`);
    console.log(`• Diversidade de preços: ${precosUnicos.length > 1 ? '✅' : '❌'}`);
    
    const sistemaOK = teste1.sucesso && teste2.sucesso && teste3.sucesso && precos.length > 0;
    
    if (sistemaOK) {
      console.log('');
      console.log('🎉 SISTEMA HÍBRIDO FUNCIONANDO PERFEITAMENTE!');
      console.log('✅ Suporta preços fixos padrão E personalizados');
    }
    
    return {
      sistemaOK: sistemaOK,
      testes: {
        precoFixo: teste1.sucesso,
        precoPersonalizado: teste2.sucesso,
        calculoAutomatico: teste3.sucesso,
        listagem: precos.length > 0
      },
      totalPrecos: precos.length,
      diversidadePrecos: precosUnicos.length
    };
...
/**
 * NOVA FUNÇÃO: Configuração completa do sistema híbrido
 */
function configurarSistemaHibrido() {
  console.log('🔧 CONFIGURANDO SISTEMA HÍBRIDO DE PREÇOS...');
  ...
  try {
    ...
    // Adicionar rotas principais com preço fixo
    const rotasPrincipais = adicionarRotasPrincipais();
    console.log('✅ Rotas principais adicionadas:', rotasPrincipais.sucessos);
    
    // Adicionar alguns preços personalizados de exemplo
    const exemploPersonalizado1 = adicionarPrecoFixoPersonalizado(
      'Lisboa Centro', 'Sintra', 8.00, 22.00, 
      { observacoes: 'Exemplo preço personalizado - rota turística' }
    );
    
    const exemploPersonalizado2 = adicionarPrecoFixoPersonalizado(
      'Porto', 'Aveiro', 12.00, 38.00,
      { observacoes: 'Exemplo preço personalizado - rota longa' }
    );
    
    console.log('✅ Exemplos personalizados adicionados');
    
    // Testar sistema
    const teste = testarSistemaPrecoHibrido();
    
    if (teste.sistemaOK) {
      console.log('🎉 CONFIGURAÇÃO COMPLETA - SISTEMA HÍBRIDO PRONTO!');
      console.log('💡 O sistema agora suporta:');
      console.log('   • Preços fixos padrão (€35)');
      console.log('   • Preços personalizados');
      console.log('   • Cálculo automático por tipo de serviço');
    } else {
      console.log('⚠️ Configuração com problemas');
    }
    
    return {
      sucesso: teste.sistemaOK,
      rotasPrincipais: rotasPrincipais.sucessos,
      exemploPersonalizado1: exemploPersonalizado1.sucesso,
      exemploPersonalizado2: exemploPersonalizado2.sucesso,
      sistemaTestado: teste.sistemaOK
    };
...
function onOpen() {
  ...
  ui.createMenu(`🚐 Sistema Gota d agua`)
...
/**
 * Testa envio de e-mail interativo (CORRIGIDO - VALORES GOTA D'ÁGUA)
 */
function testarEnvioEmailInterativo() {
  ...
  const dadosTeste = [
    ...
    '10:00',                              // Hora
    35.00,                                // ✅ Preço Cliente (CORRIGIDO)
    6.50,                                 // ✅ Valor Gota d'água (CORRIGIDO)
    28.50,                                // ✅ Valor HUB Transfer (CORRIGIDO)
    0.00,                                 // ✅ Comissão Recepção (ZERO)
    'Dinheiro',                           // Forma Pagamento
    'Recepção',                           // Pago Para
    'Solicitado',                         // Status
    'Transfer de teste - €35 total: €6.50 Gota d\'água + €28.50 HUB Transfer', // Observações atualizadas
    ...
    if (typeof criarEmailHtmlInterativo === 'function') {
      ...
      if (temCliente && temEmojis && naoTemPrezado) {
        ...
        const dadosParaEnvio = [
          ...
          '+351999000000', 'TEST123', 'Aeroporto de Lisboa', 'Gota d agua',
          '10:00', 25.00, 7.50, 15.50, 2.00, 'Dinheiro', 'Recepção', 'Solicitado',
